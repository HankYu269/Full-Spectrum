...
def extract2D(df, option):
    ...
    # Scattered points in wavelengths of 400~700 nm; find local extrema
    if option == 'g' or option == 'm':
        ...
        if option == 'g':
            ...
            for j in range(1, len(df)):
                if (df[j, 0] > 500):
                    m_end  = j
                    break
            mdata = df[wavestart:m_end, 1:]
            waveindex, timeindex = np.unravel_index(np.argmax(mdata, axis=None), mdata.shape)
    elif option == 'o':
    ...
    zlist = zdata[waveindex, :]
    zlist = zlist*1000
    xdata = df[0, 1:]
    cri_wave = df[waveindex+wavestart, 0]
    cri_time = df[0, timeindex+1]
    cri_abs = df[waveindex+wavestart, timeindex+1]
    ...
    for i in range(len(df[0, 0:])):
        if (df[0, i] >= cri_time):
            ...
            cal_abs.append(df[waveindex+wavestart, i])
    
    try:
        fit_abs, text = one_phase_fit(cal_time, cal_abs)
    except:
        cal_time, fit_abs, text = 0, 0, str()
...
def plotting(x, z, fit_x, fit_z, wave, text):
    ...
    ax.text(0.65, 0.2, text, bbox=props, fontsize=20, transform=ax.transAxes)
