...
def one_phase_fit(df, option):
    ...
    if option == 'min':
        ...
        cri_windex, cri_tindex = np.unravel_index(np.argmin(zdata, axis=None), zdata.shape)
    elif option == 'max':
        cri_abs = np.max(zdata, axis=None)
        cri_windex, cri_tindex = np.unravel_index(np.argmax(zdata, axis=None), zdata.shape)
    cri_wave = df[cri_windex+36, 0]
    cri_time = df[0, cri_tindex+1]
    ...
    for i in range(len(df[0, 0:])):
        if (df[0, i] >= cri_time):
            ...
            cal_abs.append(df[cri_windex+36, i])
    ...
    print("critical_value: {} at {} nm, {} μs".format(
        round(cri_abs, 3), cri_wave, cri_time))
...
def extract2D(df, option):
    ...
    # Find local minimum or maximum
    if option == 'min':
        waveindex = np.unravel_index(np.argmin(zdata, axis=None), zdata.shape)[0]
    elif option == 'max':
        waveindex = np.unravel_index(np.argmax(zdata, axis=None), zdata.shape)[0]
    
    zdata = zdata[waveindex, 0:]
    zdata = savgol_filter(zdata*1000, 9, 2)
    xdata = df[0, 1:]
    wave_real = df[waveindex+36, 0]
    return xdata, zdata, wave_real
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 2:
        ...
        option = sys.argv[2]
        fit_x, fit_z = one_phase_fit(df, option)
        x, z, wave = extract2D(df, option)
    else:
