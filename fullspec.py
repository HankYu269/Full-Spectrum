...
def plot_2d(df_plot, title):
    ...
    zdata = sg2.SGolayFilter2(window_size=3, poly_order=2)(zdata)
    ...
    plt.xticks([400, 450, 500, 550, 600, 650, 700], size=14)
    plt.yticks(size=14)
    plt.title(title, y=1.03, fontfamily='arial', size=25, weight='bold')
...
def plot_3d(df_plot, title, pov):
    ...
    cb.set_label(label='Δ Abs. (AU)', fontfamily='arial', size=15, labelpad=17)
    #plt.show()
    return fig
...
def plotting(x, z, wave):
    ...
    plt.title(title, y=1.03, fontfamily='arial', size=24, weight='semibold')
...
if __name__ == '__main__':
    ...
    pts, final_delay = int(input("Nr of measurements: ")),int(input("Final integra. delay (µs): "))
    ...
    if click.confirm("Extract 2D data at a designated wavelength?", default=True):
        desigwave = int(input("Wavelength (nm): "))
        x, z, wave = designate_wave(df_plot, desigwave)
        fig = plotting(x, z, wave)
        fig.savefig(input_path+"_"+str(int(wave))+".png", bbox_inches='tight')
    if click.confirm("Extract 2D data of a designated state?", default=True):
        state = str(input("State (g, l, m, or o): "))
